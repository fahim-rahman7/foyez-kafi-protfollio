...
const menu = navItems.map((item) => (
  <li key={item.id} onMouseDown={(e) => e.preventDefault()}>
    <Link
      ...
      className={`hover:text-black px-5 py-3 mx-1`}
    >
      {item.name}
    </Link>
  </li>
    ...
    >
      <div className="navbar flex justify-between mx-auto content">
        <div className="flex items-center justify-between">
          ...
          >
            <img src={mainlogo} className="h-8 sm:h-14 rounded-2xl" alt="logo" />
        ...
        <div className="lg:flex items-center">
          ...
          <p className="">
            <Link
              className="btn btn-sm xs:btn-md sm:btn-lg btn-black"
              href="#contact"
              to={`contact`}
              smooth={true}
              duration={900}
